(() => {
  ...
  const separateChatGptAndCodex = () => {
    const heading = Array.from(document.querySelectorAll("h2")).find((item) =>
      item.textContent?.includes("为你的 AI 配置"),
    );
    const section = heading?.closest("section");
    ...
    const configureCard = (card, client) => {
      ...
      if (description) {
        description.textContent = isChatGpt
  ...
  const alignSecurityWithFigma = () => {
    ...
    const oauthCard = articleByTitle("官方 OAuth");
    const readOnlyCard = articleByTitle("只读数据访问");
    const revokeCard = articleByTitle("撤销、隔离与删除");
    const statusCard = articleByTitle("状态可见、信息最小化");
